...
def get_products_by_type(product_type):
    ...
    if product_type == "top_offers":
        ...
        query = {"discount_percentage": {"$gte": 20}}
    elif product_type == "all_time_favorites":
        # All-time favorites: Star >= 4.5
        query = {"star": {"$gte": 4.5}}
    elif product_type == "trending_now":
        # Trending products based on order count
        return get_trending_now()
    else:
        query = {}

    products = uploads_collection.find(query, {"_id": 0})  # Exclude _id
...
def fetch_products(product_type):
    ...
    return jsonify({"products": products, "type": product_type})
...
def upload_product():
    ...
    try:
        ...
        # Validate the inputs
        if not description or not units or not date or not image:
        ...
        # Example: Save data to a database (mocked here as a response)
        product_data = {
            "_id": seller_data['name']+str(len(sellers_collection.find_one({"name": seller_data["name"]}).get("products",[]))),
        ...
        def update_seller_with_product(seller_id, product_id):
            """
            Updates the seller document in the sellers_collection by adding the product_id.
            If the seller doesn't exist, it creates a new document.

            Args:
        seller_id (str): The unique identifier for the seller.
                    product_id (str): The unique identifier for the uploaded product.
            """
            # Find the seller document
            seller = sellers_collection.find_one({"name": seller_id})
            print(seller)

                # Seller exists, update their product list
            if "products" in seller:
                if product_id not in seller["products"]:
                    # Add the product_id to the existing products list
                    sellers_collection.update_one(
                        {"name": seller_id},
                        {"$push": {"products": product_id}}
                    )
            else:
                # Initialize the products field with the product_id
                sellers_collection.update_one(
                    {"name": seller_id},
                    {"$set": {"products": [product_id]}}
                )
            
        update_seller_with_product(seller_data["name"], product_data["_id"])
